...
def export_carbon_intensity(df, settings):
...
def parse_arguments(arguments):
    ...
    start_date, start_time = arguments[0].split(":")
    start_date_parts = start_date.split("-")
    start_time_parts = start_time.split("-")
    end_date, end_time = arguments[1].split(":")
    end_date_parts = end_date.split("-")
...
def export_carbon_intensity_cmd(command):
    ...
    setup_data()
    return export_carbon_intensity(settings)
...
if __name__ == "__main__":
    ...
    (uk, fr, de, ca) = setup_data()
    export_carbon_intensity(uk, settings)
